...
export default function TeacherICESEContentBrowser() {
  ...
  const generateContentMutation = useMutation({
    mutationFn: async (type: 'detailed' | 'pyq') => {
      ...
      if (type === 'pyq') {
        const prompt = `Generate AUTHENTIC ICSE Previous Year Questions with TEACHING NOTES for Grade ${selectedChapter.grade} ${selectedChapter.subject}.
Chapter ${selectedChapter.number}: ${selectedChapter.title}

Generate 15-20 GENUINE previous year questions from ICSE board examinations (2015-2024) with detailed teaching guidance.
...
1. Each question MUST include:
   ...
   - Teaching notes for explaining this question type
...
3. Format each question as:
   **Year: ICSE [YEAR] | Marks: [X] | Difficulty: [LEVEL]**
   Question: [Full question text]
   
   **Teaching Notes:**
   [Key concepts tested, common mistakes, teaching strategy]
   
   **Model Answer:**
   [Detailed step-by-step solution with marking scheme]
...
4. Include exam pattern analysis
5. Provide marking scheme breakdown
6. Suggest similar practice questions
7. Add tips for teaching these question types
8. Include time management advice for students
    ...
    return (
      <View key={type} style={styles.subjectSection}>
        ...
        {subjects.map((subject) => (
          <View key={subject.id} style={[styles.subjectCard, { borderLeftColor: color, backgroundColor: colors.cardBg }]}>
            <View style={styles.subjectHeader}>
              <Text style={[styles.subjectName, { color: colors.text }]}>{subject.name}</Text>
              <Text style={[styles.chapterCount, { color: colors.textSecondary }]}>{subject.chapters.length} Chapters</Text>
            </View>
            ...
            >
              {subject.chapters.map((chapter) => (
                <TouchableOpacity
                  key={chapter.id}
                  style={[styles.chapterCard, { borderColor: color, backgroundColor: colors.inputBg }]}
                  activeOpacity={0.7}
                ...
                >
                  ...
                  <Text style={[styles.chapterCardTitle, { color: colors.text }]} numberOfLines={3}>
                    {chapter.title}
                  </Text>
                  <Text style={[styles.chapterCardDescription, { color: colors.textSecondary }]} numberOfLines={2}>
                    {chapter.description}
                  </Text>
                  <View style={[styles.generateIconContainer, { borderTopColor: colors.border }]}>
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top, backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>ICSE Teaching Material</Text>
      ...
      >
        <View style={[styles.infoBox, { backgroundColor: colors.infoBg, borderLeftColor: colors.infoBorder }]}>
          <BookOpen size={20} color={colors.infoBorder} />
          <Text style={[styles.infoBoxText, { color: colors.infoText }]}>
            ...
            {generateContentMutation.isPending && (
              <View style={[styles.loadingCard, { backgroundColor: colors.cardBg }]}>
                <Text style={[styles.loadingText, { color: colors.textSecondary }]}>
                  {contentType === 'pyq' 
                    ? 'Generating previous year questions with teaching notes...'
                    : 'Generating comprehensive teaching material...'}
                </Text>
              </View>
            )}
            ...
            {generatedContent && (
              <View style={[styles.contentCard, { backgroundColor: colors.cardBg }]}>
                <View style={[styles.contentHeader, { borderBottomColor: colors.border }]}>
                  <Text style={[styles.contentHeaderTitle, { color: colors.text }]}>
                    {contentType === 'pyq' ? 'Previous Year Questions & Teaching Notes' : 'Teaching Material'}
                  </Text>
                ...
                <Text style={[styles.contentText, { color: colors.text }]}>{cleanMarkdown(generatedContent)}</Text>
              </View>
            )}
          </ScrollView>
        </View>
