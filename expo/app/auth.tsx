...
export default function AuthScreen() {
  ...
  const { signUp, signIn, signInWithCode, continueAsGuest } = useAuth();
        ...
        >
          ...
          <Animated.View style={[styles.titleSection, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
            <Text style={styles.title}>
              {step === 'codeLogin' ? 'Quick Sign In' : isSignUp ? 'Create Account' : 'Welcome Back'}
            </Text>
            <Text style={styles.subtitle}>
              {step === 'codeLogin' ? 'Enter your 4-digit auth code' : isSignUp ? 'Join the learning revolution' : 'Continue your journey'}
            </Text>
          </Animated.View>
