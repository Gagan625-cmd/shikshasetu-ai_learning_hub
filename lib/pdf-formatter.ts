...
const separateAnswerKey = (content: string): { questions: string; answerKey: string } => {
  const answerKeyPatterns = [
    /\n(?=#{1,4}\s*(?:ANSWER\s*KEY|Answer\s*Key|ANSWERS|Answers|SOLUTION|Solutions|MODEL\s*ANSWERS))/i,
    /\n(?=\*\*(?:ANSWER\s*KEY|Answer\s*Key|ANSWERS|Answers|SOLUTIONS|Solutions|MODEL\s*ANSWERS)\*\*)/i,
    /\n(?=[═━─]{3,}\s*\n\s*(?:ANSWER\s*KEY|Answer\s*Key|ANSWERS|Answers|SOLUTIONS|Solutions))/i,
    /\n(?=(?:ANSWER\s*KEY|ANSWER KEY|Answer Key)\s*(?:\n|:))/i,
  ];
  ...
  for (const pattern of answerKeyPatterns) {
    ...
    if (match !== -1) {
      return {
        questions: content.substring(0, match).trim(),
        answerKey: content.substring(match).trim(),
      };
    }
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      line.includes('answer sheet') ||
      line.includes('model answers') ||
      line.includes('solutions:') ||
      (line.startsWith('answers') && (line.endsWith(':') || line.endsWith('key')))
    ) {
      return {
        questions: lines.slice(0, i).join('\n').trim(),
        answerKey: lines.slice(i).join('\n').trim(),
      };
    }
  }

  return { questions: content, answerKey: '' };
};

export const generatePdfHtml = (content: string, options: PdfOptions): string => {
  const accent = options.accentColor || '#1e40af';
  const isQPOrWorksheet = options.contentType === 'questionpaper' || options.contentType === 'worksheet';
  
  let formattedBody = '';
  let formattedAnswerKey = '';
  
  if (isQPOrWorksheet) {
    const { questions, answerKey } = separateAnswerKey(content);
    formattedBody = markdownToHtml(questions);
    if (answerKey) {
      formattedAnswerKey = markdownToHtml(answerKey);
    }
  } else {
    formattedBody = markdownToHtml(content);
  }
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .mcq-option:nth-child(odd) { background: #f1f5f9; }
...
<body>
  ...
  <div class="answer-key-section">
    ...
    <div class="answer-key-header">
      ...
      <div class="answer-key-divider"></div>
    </div>
